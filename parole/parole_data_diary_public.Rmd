...
### The financial cost
If the parole rates under the full board and under a partial board were the same at about 62%, we should see more prisoners get out. We estimated the number of individuals affected to be the difference in parole rate (62.61% - 56.23%) times the number of people who had a hearing with absent board members  `r with_motion$id_number %>% n_distinct()`. It means about 254 people. 
```{r}
with_motion$id_number %>% n_distinct()
```

When estimating the financial cost of the absences, we use 250 times the FY2021 per diem, which according to NDCS spokesperson is `$11,550.62` for the whole year, or `$31.65` per day. Based on audio recordings and data, these prisoners typically are deferred for a month when it was an absence issue, that's 31.65*254*30 = 241,173. If we calculate it with the lower `$9,408.32` per diem rate in FY2019 , or `$25.77` per day, it still amounts to 25.77*254*30 = 196,367.